/**
 * Build the PasswordGenerator Module Using the 
 * {@link https://github.com/umdjs/umd/blob/master/returnExports.js | UMD} Pattern.
 */
;(function ( root, factory ) {
	/**
	 * Put the PasswordGenerator in the global namespace
	 * for when not used as an AMD or CommonJS Module
	 */
	...
	if (typeof define === 'function' && define.amd) {
		...
		define([], function () {
			return root.PasswordGenerator;
		});
	} else if (typeof exports === 'object') {
		// Node. Does not work with strict CommonJS, but
		// only CommonJS-like environments that support module.exports,
		// like Node.
		module.exports = root.PasswordGenerator;
	}
}(this, function () {
	'use strict';
	var currentRules = {},
	 ...
	 */
	function PasswordGenerator(newRules){
		setRules( newRules );
	}
	...
	PasswordGenerator.prototype.getRandomSymbol = getRandomSymbol;
	...
	/**
	 * Get upto 32 (or symbol table length) random bits
	 * Inspiration from:
	 * {@link https://github.com/bitwiseshiftleft/sjcl/blob/master/core/random.js}
	 */
	function getRandomBits(){
	
		var crypto, randomArray, random32Bits;
		
		/**
		 * Try to get Node crypto
		 */
		...
		try {	
			if (typeof module !== "undefined" && module.exports && ( crypto = getCryptoModule() ) && crypto.randomBytes) {
				random32Bits = crypto.randomBytes( 32 ).readUInt32BE( 0 );
				...
				random32Bits = randomArray[0];
			}
		} catch ( e ) {
			return null;
		}
		return (random32Bits & bitMask())>>>0;
	}
	 ...
	 * is a power of two
	 */ 
	function getNextLowestPowerOfTwo( x ){
		x = (x | (x >>> 1) ) >>> 0;
		x = (x | (x >>> 2) ) >>> 0;
		x = (x | (x >>> 4) ) >>> 0;
		x = (x | (x >>> 8) ) >>> 0;
...
/**
 * Run the generator when the script is called directly 
